...
class Devis(SoftDeleteModel):
    ...
    def calculer_totaux(self):
        """Calcule les totaux HT, TVA et TTC à partir des lignes actives"""
        lignes = list(self.lignes.filter(deleted_at__isnull=True))
    ...
    def delete(self, *args, **kwargs):
        """Soft delete en cascade (bulk)"""
        now = timezone.now()
        self.lignes.filter(deleted_at__isnull=True).update(deleted_at=now)
        self.historique.filter(deleted_at__isnull=True).update(deleted_at=now)
